...
class Bola:
    def __init__(self, canvas, Barra, color):
        self.canvas = canvas
        self.Barra = Barra
        self.id = canvas.create_oval(0, 0, 15, 15, fill=color)  #tamanho da bola
        self.canvas.move(self.id, 245, 200)  #onde a bola começa

        starts_x = [-3, -2, -1, 1, 2, 3]        
        random.shuffle(starts_x)        #mudar os elementos da lista de lugar randon

        self.x = starts_x[0]   #movimento aleátorio eixo x pra começar
        self.y = -3  #bola começando pra baixo
